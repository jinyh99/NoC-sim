...
int SIM_crossbar_record(SIM_power_crossbar_t *xb, int io, LIB_Type_max_uint new_data, LIB_Type_max_uint old_data, u_int new_port, u_int old_port)
{
  switch (xb->model) {
    case MATRIX_CROSSBAR:
	 if (io)	/* input port */
...
static double SIM_crossbar_stat_energy(SIM_power_crossbar_t *crsbar, int print_depth, char *path, int max_avg, double n_data)
{
  ...
  switch (crsbar->model) {
    ...
    case MULTREE_CROSSBAR:
	 ...
	 /* static power */
	 Estatic = crsbar->I_static * Vdd * Period * orion::SCALE_S;
...
int SIM_arbiter_record(SIM_power_arbiter_t *arb, LIB_Type_max_uint new_req, LIB_Type_max_uint old_req, u_int new_grant, u_int old_grant)
{
  switch (arb->model) {
    case MATRIX_ARBITER:
...
int SIM_router_power_init(SIM_power_router_info_t *info, SIM_power_router_t *router)
{
  ...
  if (info->crossbar_model) {
    ...
    /* static power */
    router->I_static += router->crossbar.I_static;
  }
  ...
  if (info->in_buf) {
    ...
    /* static power */
    router->I_static += router->in_buf.I_static * info->n_in;
  }
...
double SIM_router_stat_energy(SIM_power_router_info_t *info, SIM_power_router_t *router, int print_depth, char *path, int max_avg, double e_fin, int plot_flag, double freq)
{
  ...
  /* static power */
  Estatic = router->I_static * Vdd * Period * orion::SCALE_S;
  SIM_print_stat_energy(SIM_power_strcat(path, "static energy"), Estatic, next_depth);
  SIM_power_res_path(path, path_len);
  Eavg += Estatic;
