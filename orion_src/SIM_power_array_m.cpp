...
int SIM_power_array_dec( SIM_power_array_info_t *info, SIM_power_array_t *arr, SIM_array_port_state_t *port, LIB_Type_max_uint row_addr, int rw )
{
  if ( ! IS_FULLY_ASSOC( info )) {
...
int SIM_power_array_data_read( SIM_power_array_info_t *info, SIM_power_array_t *arr, LIB_Type_max_uint data )
{
  if (info->data_end == 1) {
...
int SIM_power_array_data_write( SIM_power_array_info_t *info, SIM_power_array_t *arr, SIM_array_set_state_t *set, u_int n_item, u_char *data_line, u_char *old_data, u_char *new_data )
{
  u_int i;
...
/* set only used by fully-associative array */
int SIM_power_array_tag_read( SIM_power_array_info_t *info, SIM_power_array_t *arr, SIM_array_set_state_t *set )
{
  if ( IS_FULLY_ASSOC( info )) {
...
int SIM_power_array_tag_update( SIM_power_array_info_t *info, SIM_power_array_t *arr, SIM_array_port_state_t *port, SIM_array_set_state_t *set )
{
  u_int i;
...
/* only used by non-RF array */
int SIM_power_array_tag_compare( SIM_power_array_info_t *info, SIM_power_array_t *arr, SIM_array_port_state_t *port, LIB_Type_max_uint tag_input, LIB_Type_max_uint col_addr, SIM_array_set_state_t *set )
{
  int miss = 0;
  u_int i;
...
int SIM_power_array_output( SIM_power_array_info_t *info, SIM_power_array_t *arr, u_int data_size, u_int length, void *data_out, void *data_all )
{
  u_int i, j;
...
double SIM_reg_stat_energy(SIM_power_array_info_t *info, SIM_power_array_t *arr, double n_read, double n_write, int print_depth, char *path, int max_avg)
{
  ...
  /* static power */
  Estatic = arr->I_static * Vdd * Period * orion::SCALE_S;
