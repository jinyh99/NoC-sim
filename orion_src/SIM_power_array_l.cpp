 ...
 * Return value: 0
 */
int SIM_array_dec_record( SIM_power_dec_t *dec, LIB_Type_max_uint prev_addr, LIB_Type_max_uint curr_addr )
{
  ...
  if ((wordline->model = model) && model < WORDLINE_MAX_MODEL) {
    ...
    switch ( model ) {
      ...
      case CACHE_RW_WORDLINE:
	   ...
	   /* static power */
	   /* input driver */
	   wordline->I_static = (Woutdrivern * NMOS_TAB[0] + Woutdriverp * PMOS_TAB[0]) / PARM(TECH_POINT) * 100;
	   break;
 ...
 */
int SIM_array_wordline_record( SIM_power_wordline_t *wordline, int rw, LIB_Type_max_uint n_switch )
{
    ...
    switch ( model ) {
      case RW_BITLINE:
	   if ( end == 2 )
	     ...
	     /* static power */
	     bitline->I_static = 2 * (Wdecinvn * NMOS_TAB[0] + Wdecinvp * PMOS_TAB[0]) / PARM(TECH_POINT) * 100;
           }
...
int SIM_array_bitline_record( SIM_power_bitline_t *bitline, int rw, u_int cols, LIB_Type_max_uint old_value, LIB_Type_max_uint new_value )
{
...
int SIM_array_amp_record( SIM_power_amp_t *amp, u_int cols )
{
  amp->n_access += cols;
...
int SIM_array_comp_global_record( SIM_power_comp_t *comp, LIB_Type_max_uint prev_value, LIB_Type_max_uint curr_value, int miss )
{
  if ( miss ) comp->n_miss ++;
...
int SIM_array_comp_local_record( SIM_power_comp_t *comp, LIB_Type_max_uint prev_tag, LIB_Type_max_uint curr_tag, LIB_Type_max_uint input, int recover )
{
  u_int H_dist;
  int mismatch;
...
int SIM_array_mux_record( SIM_power_mux_t *mux, LIB_Type_max_uint prev_addr, LIB_Type_max_uint curr_addr, int miss )
{
...
int SIM_array_outdrv_global_record( SIM_power_out_t *outdrv, LIB_Type_max_uint data )
{
  u_int n_1;
...
int SIM_array_outdrv_local_record( SIM_power_out_t *outdrv, LIB_Type_max_uint prev_data, LIB_Type_max_uint curr_data, int recover )
{
  if ( recover )
  ...
  if ((mem->model = model) && model < MEM_MAX_MODEL) {
    ...
    switch ( model ) {
      ...
      default:	/* NORMAL_MEM */
	   ...
	   mem->I_static = I_static / PARM(TECH_POINT) * 100;
           break;
    }
...
int SIM_array_mem_record( SIM_power_mem_t *mem, LIB_Type_max_uint prev_value, LIB_Type_max_uint curr_value, u_int width )
{
...
static int SIM_array_pre_init( SIM_power_arr_pre_t *pre, int model, double pre_size )
{
  ...
  if ((pre->model = model) && model < PRE_MAX_MODEL) {
    ...
    /* static power */
    pre->I_static = n_gate * pre_size * PMOS_TAB[0] / PARM(TECH_POINT) * 100;
...
int SIM_array_pre_record( SIM_power_arr_pre_t *pre, LIB_Type_max_uint n_charge )
{
