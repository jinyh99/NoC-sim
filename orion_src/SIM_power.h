...
#define Cwordmetal    (1.8e-15 * (CSCALE * ASCALE) * orion::SCALE_M)

/* corresponds to 16um of m2 @ 275ff/um */
#define Cbitmetal     (4.4e-15 * (CSCALE * ASCALE) * orion::SCALE_M)
...
#define Period          ((double)1/(double)orion::PARM(Freq))
...
#define Powerfactor	(orion::PARM(Freq))*Vdd*Vdd
#define EnergyFactor	(Vdd*Vdd)

#define SensePowerfactor3 (orion::PARM(Freq))*(Vbitsense)*(Vbitsense)
#define SensePowerfactor2 (orion::PARM(Freq))*(Vbitpre-Vbitsense)*(Vbitpre-Vbitsense)
#define SensePowerfactor  (orion::PARM(Freq))*Vdd*(Vdd/2)
