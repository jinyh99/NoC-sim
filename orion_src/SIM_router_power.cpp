...
int FUNC(SIM_router_power_init, SIM_power_router_info_t *info, SIM_power_router_t *router)
{
  ...
  if (PARM(in_buf)) {
    ...
    SIM_buf_set_para(&info->in_buf_info, 1, PARM(in_buf_rport), 1, PARM(in_buf_set), PARM(flit_width), outdrv);
  }	/* PARM(in_buf) */
  ...
  if (PARM(cache_in_port)) {
    ...
    SIM_buf_set_para(&info->cache_in_buf_info, 1, PARM(cache_in_buf_rport), 1, PARM(cache_in_buf_set), PARM(flit_width), outdrv);
  ...
  if (PARM(mc_in_port)) {
    ...
    SIM_buf_set_para(&info->mc_in_buf_info, 1, PARM(mc_in_buf_rport), 1, PARM(mc_in_buf_set), PARM(flit_width), outdrv);
  ...
  if (PARM(io_in_port)) {
    ...
    SIM_buf_set_para(&info->io_in_buf_info, 1, PARM(io_in_buf_rport), 1, PARM(io_in_buf_set), PARM(flit_width), outdrv);
  ...
  SIM_buf_set_para(&info->out_buf_info, 1, 1, PARM(out_buf_wport), PARM(out_buf_set), PARM(flit_width), 0);
#endif	/* PARM(out_buf) */
  ...
  SIM_buf_set_para(&info->central_buf_info, 0, PARM(cbuf_rport), PARM(cbuf_wport), PARM(cbuf_set), PARM(cbuf_width) * PARM(flit_width), 0);
  /* dirty hack */
  ...
  if (info->n_v_class > 1) {
    if (info->in_arb_model = PARM(in_arb_model)) {
      if (PARM(in_arb_model) == QUEUE_ARBITER) {
        SIM_buf_set_para(&info->in_arb_queue_info, 1, 1, 1, info->n_v_class, SIM_power_logtwo(info->n_v_class), 0);
	if (info->cache_class > 1)
          SIM_buf_set_para(&info->cache_in_arb_queue_info, 1, 1, 1, info->cache_class, SIM_power_logtwo(info->cache_class), 0);
	if (info->mc_class > 1)
          SIM_buf_set_para(&info->mc_in_arb_queue_info, 1, 1, 1, info->mc_class, SIM_power_logtwo(info->mc_class), 0);
	if (info->io_class > 1)
          SIM_buf_set_para(&info->io_in_arb_queue_info, 1, 1, 1, info->io_class, SIM_power_logtwo(info->io_class), 0);
  ...
  if ((PARM(in_port) + PARM(cache_in_port) + PARM(mc_in_port) + PARM(io_in_port)) > 2) {
    if (info->out_arb_model = PARM(out_arb_model)) {
      if (PARM(out_arb_model) == QUEUE_ARBITER) {
        ...
        SIM_buf_set_para(&info->out_arb_queue_info, 1, 1, 1, info->n_total_in - 1, line_width, 0);
    ...
    info->out_arb_ff_model = SIM_NO_MODEL;
  } /* (PARM(in_port) + PARM(cache_in_port) + PARM(mc_in_port) + PARM(io_in_port)) > 2 */
...
int FUNC(SIM_buf_power_data_write, SIM_power_array_info_t *info, SIM_power_array_t *arr, u_char *data_line, u_char *old_data, u_char *new_data)
{
