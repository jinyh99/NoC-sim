...
net_tuples = [
              ('mesh',     'xy'),
#              ('mesh',     'yx'),
#              ('mesh',    'min_adaptive_vc'),
#              ('mesh',    'min_oblivious'),
#              ('ftree',    'ftree_adaptive'),
#              ('ftree',    'ftree_random'),
             ]
net_num_cores = num_cores_in_dim * num_cores_in_dim

router_inbuf_depth = 5
router_vc = 2
...
for net_tuple in net_tuples:
  ...
  dir_name = 'results/' + net_topology + '_' + net_routing
  ...
  for wkld_synth_spatial in wkld_synth_spatials:
    ...
    while load_cur <= load_end:
      ...
      output_file_name = dir_name + '/' + str(net_num_cores) + '_' + wkld_synth_spatial + '_load' + "%.2f" % load_cur + '.out'
